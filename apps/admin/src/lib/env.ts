...
const environmentSchema = z.object({
  ...
  EMAIL_DELIVERY_ENABLED:z.enum(["true","false"]).default("false"), EMAIL_PROVIDER:z.enum(["console","resend"]).default("console"), RESEND_API_KEY:z.string().min(16).optional(), EMAIL_FROM:z.string().optional(),
}).superRefine((value,ctx)=>{if(value.APP_ENV==="production"&&(!value.RESEND_API_KEY||value.EMAIL_PROVIDER!=="resend"||value.EMAIL_DELIVERY_ENABLED!=="true"||!value.EMAIL_FROM)){ctx.addIssue({code:"custom",path:["EMAIL_DELIVERY_ENABLED"],message:"production requires Resend delivery, a key, and EMAIL_FROM"})}});
